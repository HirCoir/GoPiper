...
async function loadModels() {
  try {
    ...
    if (data.success) {
      ...
      showError('Error al cargar modelos: ' + data.error);
    }
  } catch (error) {
...
async function handleGenerate() {
  ...
  try {
    ...
    const response = await fetch(`${window.location.origin}/convert`, {
      ...
      body: JSON.stringify({
        text: text,
        modelPath: selectedModel.onnxPath,
        settings: settings
      })
    });
    const data = await response.json();
    ...
    if (data.success) {
      ...
      showSuccess(`Audio generado exitosamente (${data.sentenceCount} oraciones)`);
    } else {
      showError('Error al generar audio: ' + data.error);
    }
  } catch (error) {
...
async function loadFolderPaths() {
  try {
    // For web version, use a default path
    const onnxTtsPath = 'Documents/onnx-tts';
    
    folderPathsContainer.innerHTML = `
      <div class="folder-path-item">
        <div class="folder-info">
          <i class="fas fa-folder"></i>
          <span class="folder-path">${onnxTtsPath}</span>
          <span class="folder-status" id="status-default">Default path</span>
        </div>
      </div>
    `;
    ...
    statusElement.className = 'folder-status status-found';
  } catch (error) {
...
async function addModelFolder() {
  try {
    // For web version, show a message about manual folder configuration
    showError('En la versión web, configura las rutas de modelos manualmente en el servidor');
  } catch (error) {
...
async function rescanModels() {
  try {
    ...
    const response = await fetch(`${window.location.origin}/rescan-models`);
    const data = await response.json();
...
function startQueueStatusUpdates() {
  ...
  setInterval(async () => {
    if (!settingsPanel.classList.contains('hidden')) {
      try {
        const response = await fetch(`${window.location.origin}/queue-status`);
        const data = await response.json();
